		...
		<table id="example" class="table table-bordered align-middle text-center table-striped table-responsive">
			...
			<tbody>
				...
				<tr>
					...
					<td><?=htmlspecialchars($pd->sl_nhap)?></td>
					<td><?=htmlspecialchars($pd->price)?></td>
					<td><?=htmlspecialchars($pd->catName)?></td>
					<td>
						<?php
					...
					<td>
						<a class="btn btn-warning" href="productedit.php?productid=<?=htmlspecialchars($pd->productId)?>">Chỉnh sửa</a>
						<a class="btn btn-danger" href="?productid=<?=htmlspecialchars($pd->productId)?>" onclick="return confirm('Are you sure!');" >Xóa</a> 
					</td>	
				</tr>
				<?php endforeach ?>	
			</tbody>  
